...
export default function SalesPerformance() {
  ...
  const fetchSalesPerformanceData = async (params: {
    from: Date;
    to: Date;
    granularity: string;
  }) => {
    ...
    try {
      ...
      const res = await fetch(
        `http://localhost:5000/analytics/sales-performance?${query}`,
        {
          headers: {
  ...
  // Whenever filter changes, fetch data
  useEffect(() => {
    if (filterParams) {
      fetchSalesPerformanceData(filterParams);
    }
  }, [filterParams]);
          ...
          <Card className="p-6">
            <CardContent>
              <div className="text-center space-y-2">
                ...
                <button
                  onClick={() => filterParams && fetchSalesPerformanceData(filterParams)}
